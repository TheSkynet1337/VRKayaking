...
public class SelectionTaskMeasure : MonoBehaviour
{
	...
	public float num_of_challenges = 3;
	...
	public void StartOneTask()
	{
		...
		// objectTStartingPos = hmd.transform.position + hmd.transform.forward * 0.5f + taskUI.transform.up * 0.75f;
		targetTStartingPos = hmd.transform.position + hmd.transform.forward * 0.45f;// + hmd.transform.up * 1.2f;
	...
	public void EndOneTask()
	{
		...
		// Destroy(objectT);
		Destroy(targerT);
		if (completed_challenges < num_of_challenges)
		{
			StartOneTask();
			isTaskEnd = false;
			isTaskStart = true;
		...
		{
			locomotionTechnique.inTask = false;
